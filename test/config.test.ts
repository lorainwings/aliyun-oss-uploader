  ...
  describe('loadConfig', () => {
    ...
    it('should throw error when no config found automatically', async () => {
      ...
      try {
        ...
        if (savedEnv.OSS_REGION) process.env.OSS_REGION = savedEnv.OSS_REGION;
        if (savedEnv.OSS_ACCESS_KEY_ID) process.env.OSS_ACCESS_KEY_ID = savedEnv.OSS_ACCESS_KEY_ID;
        if (savedEnv.OSS_ACCESS_KEY_SECRET)
          process.env.OSS_ACCESS_KEY_SECRET = savedEnv.OSS_ACCESS_KEY_SECRET;
        if (savedEnv.OSS_BUCKET) process.env.OSS_BUCKET = savedEnv.OSS_BUCKET;
